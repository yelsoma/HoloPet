...
public partial class StateMachineMaker : MonoBehaviour
{
    ...
    private void CreateFolderIfMissing()
    {
        ...
        Transform bodyPartsTf = transform.Find("BodyParts");
        if (bodyPartsTf == null)
        {
            GameObject child = new GameObject("BodyParts");
            child.transform.SetParent(transform);
            child.transform.localPosition = Vector3.zero;
            bodyPartsFolder = child;
            Log("Created child object: BodyParts");
        }
        else
        {
            statesFolder = bodyPartsTf.gameObject;
        }
    }

    private void CreateMissingCollider()
    {
        Collider2D col = GetComponent<Collider2D>();
        if (col == null)
        {
            col = gameObject.AddComponent<BoxCollider2D>();
            col.isTrigger = true; // optional (depends on your use case)
            Log("Created Collider");
        }
    }
